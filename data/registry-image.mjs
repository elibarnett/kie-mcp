...
export const MODEL_REGISTRY = {
  ...
  'grok-imagine-image-2-0/text-to-image': {
    ...
    description: 'NEW (Aug 7, 2026) — xAI\'s editing-first image model, #2 on both Arena text-to-image AND image-edit leaderboards (behind only gpt-image-2). 4 cr (empirical). Unique on kie: its Task ID chains into grok_segment_map (FREE, returns NAMED regions) → grok_image_edit (4 cr, region-targeted via mask_indexs) — semantic surgical editing without drawing masks. Trained on photography/design/illustration with a typography+layout emphasis. xAI\'s own API is still "coming soon" — kie is early.',
    capabilities: ['photorealistic', 'illustration', 'editing', 'text-rendering', 'design', 'segmentation', 'latest', 'new', 'budget'],
    research: { verdict: 'xAI shipped Image 2.0 on August 7, 2026 as the Quality Mode on grok.com/imagine and the mobile apps, built around editing as a first-class capability rather than a bolt-on. xAI reports #2 in the world on BOTH Arena text-to-image and image-edit leaderboards as of launch day, behind only OpenAI\'s gpt-image-2 — a rare dual placement. The differentiator is the edit workflow: segmentation that selects precise regions, a magic-wand that changes only what you point at, background removal with transparency export, multi-reference editing (up to 5 inputs consumer-side), and smart-resize across nine aspect ratios. Training emphasized photography, design, and illustration with attention to typography and layout. On kie the surface is three chained endpoints: text-to-image (4 cr, this entry), segment-map (FREE — returns NAMED regions with mask PNGs, e.g. {index:0,"red apple"},{index:1,"wooden table"}), and image-edit (4 cr — prompt + mask_indexs against the source task). Live-verified end-to-end 2026-08-14: generate → segment (0 cr) → background-only edit succeeded. Independent evaluation beyond the Arena ranking is thin one week post-launch; kie exposes T2I + segment + edit only (no background-removal or 5-image multi-ref endpoint yet), and the multi-ref/smart-resize consumer features are absent from the API surface. Note the source-task dependency: segment-map takes a task_id only, but image-edit gained an image_urls mode (Aug 2026, live-verified) that edits ANY uploaded image — whole-image instruction edits, no segmentation.', bestFor: ['semantic surgical edits — "change the background, keep the subject" — picked by region NAME, no mask drawing', 'iterative refinement loops: generate once (4 cr), segment free, edit only the offending region (4 cr) instead of regenerating', 'design/layout/typography-forward images (posters, cards, product shots) at 4 cr', 'multi-object scenes where per-object control matters — the segment map exposes every object', 'budget alternative to Flux Kontext Pro for targeted edits (a tenth of Kontext Pro\'s price)'], weaknesses: ['REGION-targeted editing (segment-map) only works on 2.0-generated task_ids; uploaded photos get whole-image instruction edits via image_urls (no per-region masks)', 'kie exposes 3 of the consumer feature set — no background-removal, no 5-image multi-reference, no smart-resize via API yet', 'launched Aug 7 — independent benchmarks beyond the Arena ranking are scarce', 'edit results are new full images (resultUrls), not layered/transparent outputs', 'still #2 to gpt-image-2 on both boards; for absolute top text rendering use gpt-image/2-text-to-image'], promptTechniques: ['T2I: describe subject + setting + composition + design intent in prose; typography prompts ("poster with headline text: ...") play to its training', 'edit chain: run grok_segment_map first and read the region NAMES, then target mask_indexs by name — do not guess indices', 'edit prompts should describe the desired end state of the masked region and say what to preserve ("...keep the apple unchanged")', 'iterate on the SAME source task_id — each edit is 4 cr, cheaper than regenerating', 'aspect_ratio: 1:1, 2:3, 3:2, 16:9, 9:16 (docs enum) — set at generation time'], communityInsights: ['xAI: "#2 in the world" on Arena T2I and image-edit at launch — but the arena entries are the consumer "quality" mode; kie routes the same 2.0 model', 'launch coverage universally leads with editing (magic wand, segmentation, background removal) rather than raw generation quality', 'API access was "coming soon, no date" from xAI at launch — kie shipped it within days, so kie may be ahead of the official API'], costEfficiency: '4 cr per generation and 4 cr per edit, segment-map free (empirical 2026-08-14 — 4.0 + 0.0 + 4.0 exactly). A generate→segment→edit loop is 8 cr total vs. ~10 cr for a single flux-kontext-pro edit, and the free segmentation makes multi-round refinement unusually cheap. Same price as Grok 1.x T2I for a much stronger model — no reason to use grok-imagine/text-to-image over this.', comparedTo: { 'grok-imagine/text-to-image': 'Same 4 cr; 2.0 is the successor with far stronger quality (#2 Arena) and the segment/edit chain — prefer 2.0.', 'flux-kontext-pro': 'Kontext edits UPLOADED photos with text instructions (no segmentation) at ~10 cr; Grok 2.0 edits its own generations by named region at 4 cr. Different starting points — Kontext for existing photos, Grok 2.0 for generate-then-refine.', 'gpt-image/2-text-to-image': 'gpt-image-2 is #1 on both Arena boards and better for dense text; Grok 2.0 is #2 at a fraction of the price with the free segment map.', 'nano-banana-pro': 'NB Pro is the reasoning/complex-prompt king and edits uploads via multi-image composition; Grok 2.0 wins on cheap region-targeted refinement of its own output.' }, lastResearched: '2026-08-14', sources: ['https://www.unite.ai/xai-ships-grok-imagine-image-2-0-with-precise-editing-and-a-top-arena-ranking/', 'https://www.testingcatalog.com/xai-launches-imagine-image-2-0-in-grok-quality-mode/', 'https://docs.kie.ai/market/grok-imagine-image-2-0/text-to-image', 'https://docs.kie.ai/market/grok-imagine-image-2-0/segment-map', 'https://docs.kie.ai/market/grok-imagine-image-2-0/image-edit'] },
    type: 'market',
